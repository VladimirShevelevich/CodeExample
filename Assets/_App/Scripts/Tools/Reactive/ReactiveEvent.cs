...
namespace _App.Scripts.Tools.Reactive
{
    public class ReactiveEvent<T> : IReadOnlyReactiveEvent<T>, IDisposable
    {
        ...
        {
            return _property.Where(predicate);
        }
    }
    ...
    public class ReactiveEvent<T1, T2> : IReadOnlyReactiveEvent<T1, T2>, IDisposable
    {
        ...
        public IObservable<(T1, T2)> SkipLatestValue()
        {
            return _property.SkipLatestValueOnSubscribe().Select(entry => (entry.arg1, entry.arg2));
        }

        private IDisposable Subscribe(Action<Entry> entry)
        {
